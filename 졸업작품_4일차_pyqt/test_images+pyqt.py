...
def grab(cam, queue):
    ...
    rawCapture = PiRGBArray(capture, size=(640, 480))
    for frame in capture.capture_continuous(rawCapture, format="bgr", use_video_port=True):
...
capture_thread = threading.Thread(target=grab, args=(0, q))
